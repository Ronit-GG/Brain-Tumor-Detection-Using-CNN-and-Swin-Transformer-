...
\centering
\vspace*{1.5cm}
{\LARGE\bfseries Brain Tumor Detection from MRI Images using an Ensemble of CNN, Transfer Learning and Swin Transformer Models\par}
\vspace{1.2cm}
8th Semester Project Report\\[4pt]
submitted to\\[4pt]
TECHNO INDIA UNIVERSITY\\[4pt]
In partial fulfilment of the requirements for the award of the degree of\\[4pt]
Bachelor of Technology\\[4pt]
in\\[4pt]
...
<STUDENT\_NAME\_4> (Roll No. <ROLL\_4>)\\[4pt]
Under the guidance of\\[4pt]
<SUPERVISOR\_NAME>, <SUPERVISOR\_DESIGNATION>\\[4pt]
Department of <DEPARTMENT\_NAME>\\[4pt]
<UNIVERSITY\_NAME>\\[4pt]
<SUBMISSION\_MONTH\_YEAR>\\[4pt]
\end{titlepage}
\pagenumbering{roman}
\tableofcontents
\listoffigures
...
We hereby declare that the project report entitled "Brain Tumor Detection from MRI Images using an Ensemble of CNN, Transfer Learning and Swin Transformer Models" is an authentic record of our own work carried out at the Department of <DEPARTMENT\_NAME>, <UNIVERSITY\_NAME>, during the 8th semester of the academic year <ACADEMIC\_YEAR> under the supervision of <SUPERVISOR\_NAME>, <SUPERVISOR\_DESIGNATION>.
We further declare that the matter embodied in this project report has not been submitted by us for the award of any other degree or diploma of this or any other Institute or University. All the information has been obtained and presented in accordance with academic rules and ethical conduct. We also declare that, as required by these rules and conduct, we have fully cited and referenced all materials and results that are not original to this work. The dataset used in this project is publicly available under its original license; no patient-identifiable data was collected, generated, or stored.
Place: <UNIVERSITY\_ADDRESS>
Date: <SUBMISSION\_MONTH\_YEAR>
Signatures:
\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_          \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
<STUDENT\_NAME\_1>              <STUDENT\_NAME\_2>
Roll No: <ROLL\_1>              Roll No: <ROLL\_2>
\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_          \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
...
This is to certify that the Dissertation Report entitled "Brain Tumor Detection from MRI Images using an Ensemble of CNN, Transfer Learning and Swin Transformer Models" submitted by <STUDENT\_NAME\_1>, <STUDENT\_NAME\_2>, <STUDENT\_NAME\_3>, and <STUDENT\_NAME\_4> to <UNIVERSITY\_NAME> is a record of bonafide project work carried out by them under my supervision and guidance, and is worthy of consideration for the award of the degree of Bachelor of Technology (B.Tech) in <SPECIALIZATION>.
<SUPERVISOR\_NAME>,
Project Supervisor,
...
We would like to first express our sincere gratitude to our project supervisor, <SUPERVISOR\_NAME>, <SUPERVISOR\_DESIGNATION>, Department of <DEPARTMENT\_NAME>, <UNIVERSITY\_NAME>. Their continued guidance, technical insight, and constructive feedback during every stage of this project were instrumental in shaping the final outcome. Their willingness to discuss difficult engineering decisions, including the diagnosis of subtle numerical-stability issues during mixed-precision training, helped us approach the work with both rigour and curiosity.
We take this opportunity to express our gratitude to all faculty members of the Department of <DEPARTMENT\_NAME> for their support, the encouragement they provided during our coursework, and the foundation they laid in machine learning, computer vision, and software engineering.
...
Finally, we thank our parents and families for their unceasing encouragement, patience, and support throughout the semester.
\chapter*{Abstract}
...
\addcontentsline{toc}{section}{1.1 Medical Imaging and Computer-Aided Diagnosis}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{1.2 Objectives of the Project}
\subsection*{1.2.1 Primary Objective}
\addcontentsline{toc}{subsection}{1.2.1 Primary Objective}
...
\addcontentsline{toc}{subsection}{1.2.2 Secondary Objectives}
...
\addcontentsline{toc}{section}{1.3 Challenges in Brain Tumor Classification}
...
\addcontentsline{toc}{section}{1.4 Methodologies}
...
\begin{itemize}[leftmargin=*]
  \item Data Collection: The Brain Tumour MRI Data Set (Nickparvar, Kaggle) is adopted, which includes 7,200 labelled MRI images divided into four categories. The data set is randomly split into training (4,480 images), validation (1,120 images), and test sets (1,600 images) using stratification and a predefined seed value to ensure consistency. More information can be found in Chapter 3.
...
\addcontentsline{toc}{section}{2.1 Introduction to the Field}
...
\addcontentsline{toc}{section}{2.2 Review of Related Work}
\subsection*{2.2.1 Babu Vimala et al. (2023): Hybrid Deep Learning with EfficientNet Variants}
\addcontentsline{toc}{subsection}{2.2.1 Babu Vimala et al. (2023): Hybrid Deep Learning with EfficientNet Variants}
...
\addcontentsline{toc}{subsection}{2.2.2 Islam et al. (2024): BrainNet Optimised EfficientNet Architecture}
...
\addcontentsline{toc}{section}{2.3 Summary of Reviewed Approaches}
...
\addcontentsline{toc}{section}{2.4 Market Research}
...
\addcontentsline{toc}{section}{2.5 Research Gap and Limitations of Existing Systems}
Despite the large body of work, there are several limitations in the literature:
Overstated accuracy: Many studies report only validation-set accuracy without a truly held-out test set, so headline numbers are inflated. This drop in accuracy from val to test, which we find to be between 4 and 8 percentage points across our models, is rarely discussed.
Limited architectural diversity in ensembles: Most ensemble studies combine variants of the same architecture family (e.g. multiple ResNets or multiple EfficientNets). Very few works intentionally combine architecturally different model families (CNN, depthwise-separable CNN, and transformer) to maximize error diversity.
...
\addcontentsline{toc}{section}{2.6 Motivation for Project Selection}
...
\addcontentsline{toc}{chapter}{Chapter 3: Dataset and Data Preparation}
...
\addcontentsline{toc}{section}{3.2 Dataset Source and Acquisition}
...
\addcontentsline{toc}{section}{3.3 Class Categories and Clinical Meaning}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{3.4 Rationale for Dataset Selection}
...
\addcontentsline{toc}{section}{3.5 Dataset Statistics}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{3.6 Exploratory Data Analysis}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{3.7 Train / Validation / Test Split Strategy}
...
\addcontentsline{toc}{section}{3.8 Image Preprocessing Pipeline}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{3.9 Data Augmentation Strategy}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{3.10 Ethical Considerations and Licensing}
...
\addcontentsline{toc}{chapter}{Chapter 4: Project Planning}
\section*{4.1 Overview}
\addcontentsline{toc}{section}{4.1 Overview}
...
\addcontentsline{toc}{section}{4.2 Current Status of the Project}
...
\addcontentsline{toc}{section}{4.3 System Architecture}
...
Model Layer: This layer includes the three trained model frameworks (src/cnn\_model.py, src/transfer\_model.py, src/swin\_model.py), the ensemble combiner (src/ensemble.py), the Grad-CAM implementation (src/explainability.py), and the training engine (src/train.py). All the model weights are saved as .pth checkpoint files in the models/ directory, as well as an ensemble\_config.json that tracks the ensemble method and optimal weights.
Data Layer: The dataset is located in DATASET/ with folders Training/ and Testing/. Cached divided split CSVs can be found in data/processed/. Training outcomes (plots, confusion matrices, Grad-CAM images, CSV reports) are saved to outputs/. Write TensorBoard event files to logs/.
\section*{4.4 Identified Challenges and System Limitations}
\addcontentsline{toc}{section}{4.4 Identified Challenges and System Limitations}
...
\addcontentsline{toc}{section}{4.5 Strategy to Address Identified Challenges}
...
\addcontentsline{toc}{section}{4.6 Implementation Roadmap}
...
\addcontentsline{toc}{section}{4.7 Project Scheduling and Gantt Chart}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{chapter}{Chapter 5: Project Description}
\section*{5.1 Model of the software.}
\addcontentsline{toc}{section}{5.1 Model of the software.}
The Brain Tumour Detection system is developed using modular python package structure. All production code lives in src/ and is divided into modules with a single responsibility. The two user-facing interfaces are in the app/ folder. The notebooks/ folder contains seven Jupyter notebooks that reproduce the full experimental pipeline with embedded outputs for auditability.
Below is a summary of the key technology decisions and rationale:
\begin{center}
\small
\begin{tabularx}{\textwidth}{YYY}
\toprule
...
\addcontentsline{toc}{section}{5.2 Software Requirements Specification (SRS)}
5.2.1 Introduction
The Brain Tumour Detection system is a deep learning based diagnostic aid that classifies brain MRI slices into 4 categories . This SRS document describes the functional and non-functional requirements, system constraints, and expected performance characteristics
5.2.2 General description
The system is a client-server application with a backend written in python and a web-based frontend. It provides two modes of interaction, 
(1) A Streamlit web interface for interactive single image prediction with visual feedback 
(2) Programmatic batch predictions through a FastAPI REST endpoint. prediction. Both interfaces wrap the same BrainTumorPredictor class, so no matter which interface you use to access it, the results will be the same.
\subsection*{5.2.3 Functional Requirements}
\addcontentsline{toc}{subsection}{5.2.3 Functional Requirements}
The system will be:
Input: Brain MRI image in JPG, PNG or BMP format. The size of the original image is unlimited.
Preprocess input image: convert to RGB, resize to 224 x 224 and normalise with ImageNet statistics.
Feed the preprocessed image through the 3 trained models (custom CNN, EfficientNet-B0, Swin-Tiny). Output the softmax probability vectors for each.
Then, combine the three probability vectors with saved ensemble method and weights to get a single ensemble probability distribution.
Return predicted class (class with highest ensemble probability) and confidence score of this class.
Generate (optionally) Grad-CAM heatmap overlays for the predicted class for each base model.
Flag predictions with less than a user configurable threshold of confidence (default 0.85) with "consult radiologist" advice.
Output to Streamlit UI: predicted class (colour coded), confidence, probability bar chart, probability table per model, 3 Grad-CAM heatmap images. Expose a /predict REST endpoint that takes a multipart/form-data image upload and returns a JSON object with the predicted class, confidence, and per-model probabilities.
\subsection*{5.2.4 Interface Requirements}
\addcontentsline{toc}{subsection}{5.2.4 Interface Requirements}
User Interface:
Web-based interface, accessible in any modern browser.
Drag \& Drop image upload or select test-sample from drop down.
Model info and settings in the sidebar.
Hardware Interface:
Works on a regular desktop or laptop, with or without a dedicated GPU. Gracefully falls back to CPU.
Software Interface:
Python 3.11+, PyTorch 2.12, timm, Streamlit, FastAPI, ONNX Runtime.
\subsection*{5.2.5 Performance Requirements}
\addcontentsline{toc}{subsection}{5.2.5 Performance Requirements}
Single image inference shall be performed in 300 ms on GPU (including Grad-CAM) or 1 second on CPU.
Model loading should take less than 10 seconds at first startup.
The Streamlit interface should stay responsive during prediction.
\subsection*{5.2.6 Design Constraints}
\addcontentsline{toc}{subsection}{5.2.6 Design Constraints}
The system shall be developed using open source frameworks and tools.
All model weights will be stored locally, no external API calls are required at inference time.
The system should be deployable on standard institutional infrastructure or on personal laptops .
\subsection*{5.2.7 Non-Functional Attributes}
\addcontentsline{toc}{subsection}{5.2.7 Non-Functional Attributes}
Reliability: The system should provide reliable and deterministic predictions for the same input image. To achieve reproducibility, we will use static random seeds and cache data splits.
Security: User data will not be stored or sent anywhere else. Image inputs are processed in memory only, with no persistent storage.
Maintainability: Modularity and modular src/ folder structure allow easy modifications. The retraining and substitution of each model does not interfere in other pieces of code.
Scalability: FastAPI framework supports concurrent connections. Onnx export will allow models to be run on edge devices without the PyTorch framework.
\section*{5.3 Functional Specification}
\addcontentsline{toc}{section}{5.3 Functional Specification}
\subsection*{5.3.1 Deep Learning Models}
\addcontentsline{toc}{subsection}{5.3.1 Deep Learning Models}
\begin{figure}[H]
  \centering
...
VGG style architecture with four convolution blocks. Each block consists of two consecutive 3x3 convolutions. Batch Normalisation and ReLU activation are performed after each convolution. The 2x2 max-pooling layer halves the spatial dimensions. The number of channels doubles at each block: 32, 64, 128, 256 The feature map (256 x 14 x 14) generated by the fourth block is fed into a Global Average Pooling layer which reduces the spatial dimensions and outputs a 256-dimensional feature vector. This vector is passed through a two-layer fully-connected classifier: Dropout Linear (256 to 128) ReLU Dropout Linear (128 to 4) to produce four logits. Weight initialisation Kaiming normal (mode=fan\_out, relu) Total params: 1,206,628
Model 2: EfficientNet-B0 Transfer Learning (src/transfer\_model.py)
We selected EfficientNet-B0 over ResNet-50, EfficientNet-B3, DenseNet-121 and ConvNeXt-Tiny due to its accuracy-per-parameter efficiency (77.7\% ImageNet top-1 at only 5.3 million parameters) and its fit to the 4 GB VRAM budget. The pretrained backbone is from torchvision . We replace its 1000-class classifier head by a new head: Dropout(0.3) -> Linear(1280 to 4).
Training happens in two steps. Stage 1 (warm-up, 4 epochs, learning rate = 0.001): The backbone is frozen and only the 5,124 head parameters are updated with gradients . The module's .train() method is overridden to keep the frozen backbone in .eval() mode to avoid BatchNorm running-statistic drift. Unfreeze backbone Stage 2 (fine-tune, 8 epochs, learning rate = 0.0001) The 10x smaller learning rate helps mitigate catastrophic forgetting of pretrained features. Total parameters: 4,012,672.
Model 3: Swin-Tiny Transformer (src/swin\_model.py)
Swin-Tiny (swin\_tiny\_patch4\_window7\_224 from timm library) is a hierarchical vision transformer that takes as input an image divided into 4 x 4 patches and then processes them through four stages of windowed self-attention blocks. Each stage reduces the spatial resolution via patch merging (analogous to pooling in CNNs) while increasing the feature dimensionality. The main breakthrough of Swin is the shifted-window mechanism, where attention windows are shifted by half a window size in alternating layers, allowing information to flow between windows without incurring the quadratic costs of global attention.
The classifier head is replaced the same way as EfficientNet (Dropout 0.3, Linear 768 to 4). Two-stage training follows the transfer-learning recipe, but with a much smaller Stage 2 learning rate. of 0.00002 (50 times smaller than Stage 1) since transformer attention weights are very sensitive to large gradient updates. Total parameters: 27,522,430.
...
\addcontentsline{toc}{subsection}{5.3.2 Mathematical Model and Loss Function}
All three models are trained with the standard cross-entropy loss. For a single sample with true class y and predicted probability p\_y for that class, the loss is:
    L=-log(p\_y)
where p\_y is the probability assigned by softmax to the correct class. The loss is not class-weighted because the four classes are perfectly balanced.
...
\addcontentsline{toc}{subsection}{5.3.3 Ensemble Decision Logic}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{subsection}{5.3.4 Grad-CAM Explainability Generation}
\begin{figure}[H]
  \centering
...
For the custom CNN and EfficientNet-B0, the target layer is the last convolutional layer before global average pooling. For Swin-Tiny without the conventional convolutional layers, the goal is the LayerNorm at the end of the last transformer stage. Swin's activations are in channels-last format (sequence\_length x channels), while Grad-CAM expects them in channels-first format (channels x height x width), so a reshape operation is required. The implementation is based on the pytorch-grad-cam library by Jacob Gildenblat.
The heatmap produced is a single channel gray-scale image where red indicates high importance and blue indicates low importance. This heatmap is superimposed on the original MRI image at 45\% opacity to form a composite visualisation where the anatomical structure and the attention pattern of the model can be viewed simultaneously by the clinician.
\section*{5.4 Design Specification}
\addcontentsline{toc}{section}{5.4 Design Specification}
\subsection*{5.4.1 Use-Case Diagrams}
\addcontentsline{toc}{subsection}{5.4.1 Use-Case Diagrams}
\begin{figure}[H]
  \centering
...
Radiologist / Clinician: Upload a patient MRI Image (or select a test sample). View the predicted class and confidence. Check Grad-CAM heatmaps to see if the model is reasoning correctly. Adjust the confidence threshold.
Researcher / Developer: Calls batch prediction API of FastAPI, looks at per-model probability breakdowns, runs evaluation scripts on custom datasets, exports models to ONNX format.
Administrator: Starts docker container, configures server, watches application logs, updates model checkpoints.
\subsection*{5.4.2 Data Flow Diagrams}
\addcontentsline{toc}{subsection}{5.4.2 Data Flow Diagrams}
\begin{figure}[H]
  \centering
...
\end{figure}
Training pipeline DFD : Data loader ( src/data\_loader.py ) takes as input the raw MRI images coming from DATASET / and applies preprocessing and augmentation transforms ( src/preprocess.py ) . The processed batches are then passed into the training engine (src/train.py) which does forward passes, computes the loss, backpropagates, and logs metrics to tensorboard. The engine evaluates on the validation set at each epoch and saves the best checkpoint to models/.
Inference Pipeline DFD User uploads an image through Streamlit UI or FastAPI endpoint. The image is passed to BrainTumorPredictor (src/inference.py) which loads the image, applies the inference transform, runs all three models in parallel, combines the softmax outputs using the ensemble combiner, and optionally does Grad-CAM overlays. The presentation layer is given the PredictionResult object to display.
\subsection*{5.4.3 Data Dictionaries}
\addcontentsline{toc}{subsection}{5.4.3 Data Dictionaries}
The following tables describe the key data structures used throughout the system.
...
\addcontentsline{toc}{chapter}{Chapter 6: Implementation Issues}
\section*{6.1 Diversity of Datasets and Image Quality}
\addcontentsline{toc}{section}{6.1 Diversity of Datasets and Image Quality}
...
\addcontentsline{toc}{section}{6.2 Model choice and hyperparameter sensitivity}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{6.3 GPU Memory Limits and Mixed-Precision}
...
\addcontentsline{toc}{section}{6.4 Class Confusion and Generalisation Gap}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{6.5 Trustworthiness in Clinical Adoption and Explainability}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{section}{6.6 Problem of Dominant Model in Ensemble}
\begin{figure}[H]
  \centering
...
\addcontentsline{toc}{chapter}{Chapter 7: Conclusion, Summary and Future Scope}
\section*{7.1 Summary of Work}
\addcontentsline{toc}{section}{7.1 Summary of Work}
...
\addcontentsline{toc}{section}{7.2 Conclusion}
...
\addcontentsline{toc}{section}{7.3 Future Scope}
